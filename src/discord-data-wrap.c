...
static void
_json_key_override(const struct discord_data_wrap *field,
                   const char **name,
                   size_t *length)
{
    if (!field || !field->tmpl || !field->tmpl->name.buf) return;

    if (field->tmpl->name.length == 6
        && 0 == memcmp(field->tmpl->name.buf, "Inline", 6))
    {
        *name = "inline";
        *length = 6;
        return;
    }
    if (field->tmpl->name.length == 7
        && 0 == memcmp(field->tmpl->name.buf, "Default", 7))
    {
        *name = "default";
        *length = 7;
        return;
    ...
    case REFLECTC_TYPES(struct): {
        ...
        if (reflectc_length(member) > 1) {
            ...
            for (size_t i = 0; i < member->members.length; ++i) {
                ...
                const char *key_name = f->tmpl->name.buf;
                size_t key_len = f->tmpl->name.length;
                _json_key_override(f, &key_name, &key_len);
